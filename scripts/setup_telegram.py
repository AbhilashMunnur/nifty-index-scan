#!/usr/bin/env python3
"""Save a Telegram bot token and chat ID to .env, then send a test message."""
...
def ask_token() -> str:
    print("\n1/2  Bot token")
    print("  Open Telegram, message @BotFather, send /newbot, and copy the token.")
...
def ask_chat_id(token: str) -> str:
    ...
    print("\n2/2  Chat ID")
    print(f"  Open @{username} and send /start (or any message).")
    ...
    print("Could not find a message. Message the bot directly with /start.")
    sys.exit(1)
...
def main() -> None:
    ...
    print("Telegram alert setup for Nifty 15-minute scans")
    token = ask_token()
    ...
    result = call(
        ...
        "sendMessage",
        chat_id=chat_id,
        text="Nifty 1H alerts connected. You will get a P&L snapshot every 15 minutes in market hours.",
    )
    if not result.get("ok"):
    ...
    print("\nTest message sent. Saved to .env.")
    print("Start 15-minute alerts with:")
    print("  python main.py --loop")
